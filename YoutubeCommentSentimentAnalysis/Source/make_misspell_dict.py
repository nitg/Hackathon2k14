...
for line in fob: #accessing individual line
	if line[0]=='$':  #because correct word start with $ for ex $america
		correct_word=line[1:].replace('\n','').replace('.','') #this tells the word that follows dollar sign is the correct word
	else:
		incorrect_word=line.replace('\n','').replace('.','') #this tells that the word that doesnt has dollar sign at start is incorrect for ex americo
		misspell[incorrect_word]=correct_word #inserts americo:america into dictionary


db.words.insert(misspell)
